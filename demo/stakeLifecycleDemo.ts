...
async function testStakeLifecycle() {
  ...
  if (balance < 0.05) {
    console.log(
      "❌ Balance too low for staking test (need ~0.05 SOL). Fund this address with devnet SOL."
    );
    return;
  }
  ...
  console.log(
    `🔗 Stake Tx: https://explorer.solana.com/tx/${stakeResult.signature}?cluster=devnet`
  );
  ...
  if (unstakeResult.signature) {
    console.log(
      `🔗 Unstake Tx: https://explorer.solana.com/tx/${unstakeResult.signature}?cluster=devnet`
    );
  }
  ...
  if (withdrawResult.signature) {
    console.log(
      `🔗 Withdraw Tx: https://explorer.solana.com/tx/${withdrawResult.signature}?cluster=devnet`
    );
  }
