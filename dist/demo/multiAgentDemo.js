...
async function main() {
    ...
    let resB = await agentB.invoke({
        messages: [
            new HumanMessage(`Send 0.1 SOL to ${recipientAddress}. Use get_balance first, then transfer_sol if you have enough.`),
        ],
    });
    let lastContent = resB.messages[resB.messages.length - 1]?.content;
    if (typeof lastContent === "string")
        console.log("Agent B:", lastContent);
    console.log("");
    // --- Step 2: A sends 0.1 SOL to B ---
    console.log("--- Step 2: Agent A sends 0.1 SOL to Wallet B ---\n");
    const resA = await agentA.invoke({
        messages: [
            new HumanMessage(`Send exactly 0.1 SOL to ${pubkeyB} so the other agent (Wallet B) can complete a transaction.`),
        ],
    });
    ...
    resB = await agentB.invoke({
        messages: [
            new HumanMessage(`You now have enough SOL. Send 0.1 SOL to ${recipientAddress}. Use transfer_sol.`),
        ],
    });
