...
export function createWalletTools(wallet) {
    ...
    const transferSpl = tool(async ({ mint, to, amount }) => {
    ...
    const stakeSol = tool(async ({ amount, validator }) => {
        ...
        name: "stake_sol",
        description: "Stake SOL (native staking). If no validator is provided, the wallet will automatically choose a validator.",
        schema: z.object({
    ...
    const unstakeSol = tool(async ({ stakeAccount }) => {
        const intent = { type: "unstake_sol", stakeAccount };
        const result = await wallet.execute(intent);
        if (result.success) {
            return result.signature
    ...
    const withdrawStakeSol = tool(async ({ stakeAccount, amount, to }) => {
        const intent = { type: "withdraw_stake_sol", stakeAccount, amount, to };
        const result = await wallet.execute(intent);
        if (result.success) {
            return result.signature
    ...
    const whirlpoolSwap = tool(async ({ poolAddress, mint, inputAmount, outputAmount, slippageToleranceBps }) => {
        ...
        name: "whirlpool_swap",
        description: "Swap tokens on Orca Whirlpools (concentrated liquidity AMM). Use devnet pool and mint addresses. Provide either inputAmount (exact-in) or outputAmount (exact-out) in token native units.",
        schema: z.object({
    ...
    const whirlpoolAddLiquidity = tool(async ({ poolAddress, tokenAmountA, tokenAmountB, slippageToleranceBps, mode, lowerPrice, upperPrice }) => {
        const intent = {
        ...
        description: "Add liquidity to an Orca Whirlpool to earn fees. Use full_range for full-range LP or concentrated with lowerPrice/upperPrice. Amounts in token native units.",
        schema: z.object({
            poolAddress: z.string().describe("Orca Whirlpool address (base58)"),
    ...
    const whirlpoolHarvest = tool(async ({ positionMintAddress }) => {
        const intent = { type: "whirlpool_harvest", positionMintAddress };
        const result = await wallet.execute(intent);
        if (result.success) {
            return result.signature
